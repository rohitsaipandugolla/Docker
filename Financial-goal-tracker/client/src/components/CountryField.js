...
function countryToCode(country) {
    ...
    for (let count = 0; count < countries.length; count++) {
        if (country.toLowerCase() === countries[count].label.toLowerCase()) {
            return countries[count].code;
        }
...
export default function CountrySelect(props) {
    ...
    const [country, setCountry] = React.useState({ code: countryToCode(props.country), label: props.country });

    const handleChange = (event)=>{
        if(event.target.innerText){
        var countryName = event.target.innerText.split("(")[0].substring(4).trim();
        setCountry({code:countryToCode(countryName),label:countryName})
    ...
    return (
        <div>
            ...
            <InputLabel  htmlFor="country" className={classes.labelSize} >
                Country
          </InputLabel>
            ...
            <FormControl>
                ...
                <Autocomplete
                    ...
                    renderInput={(params) => (
                        <TextField className={classes.formtextField} style={{ color: "red" }}
                            size="14px"
                            {...params}
                            variant="outlined"
                            inputProps={{
