package com.sample.financialgoaltracker.service;

import com.sample.financialgoaltracker.dto.BudgetComponentDTO;
import com.sample.financialgoaltracker.entity.Budget;
import com.sample.financialgoaltracker.entity.BudgetComponent;
import com.sample.financialgoaltracker.mapper.BudgetComponentMapper;
import com.sample.financialgoaltracker.repository.BudgetComponentRepository;
import org.junit.Before;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.mockito.Mockito;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.jdbc.AutoConfigureTestDatabase;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.test.context.junit4.SpringRunner;

import java.util.ArrayList;
import java.util.List;
import java.util.Optional;

import static org.assertj.core.api.Assertions.assertThat;

@AutoConfigureTestDatabase(replace= AutoConfigureTestDatabase.Replace.NONE)
@RunWith(SpringRunner.class)
@SpringBootTest
public class BudgetComponentServiceTest {

    @MockBean
    private BudgetComponentRepository budgetComponentRepository;

    @Autowired
    private BudgetComponentServiceImpl budgetComponentServiceImpl;

    @Autowired
    private BudgetComponentMapper budgetComponentMapper;

    private BudgetComponent budgetComponent1;
    private BudgetComponent budgetComponent2;

    @Before
    public void setUp(){
        budgetComponent1 = new BudgetComponent();
        budgetComponent2 = new BudgetComponent();

        budgetComponent1.setCategory("sales");
        budgetComponent1.setCurrency(1000.00);
        budgetComponent1.setCurrencyFormat("Indian Rupee");
        budgetComponent1.setFrequency("monthly");
        budgetComponent1.setCreatedAt("2020-05-11");
        budgetComponent1.setCreatedBy("ray");
        budgetComponent1.setModifiedAt("2020-05-16");
        budgetComponent1.setModifiedBy("ray");
        budgetComponent1.setDeleted(false);
        budgetComponent1.setActive(false);

        Budget budget1 = new Budget();
        budget1.setStartDate("2020-05-01");
        budget1.setEndDate("2020-12-01");
        budget1.setFrequency("monthly");
        budget1.setCreatedAt("13:45");
        budget1.setCreatedBy("ray");
        budget1.setModifiedAt("15:45");
        budget1.setModifiedBy("ray");
        budget1.setDeleted(false);
        budget1.setActive(false);

        budgetComponent1.setBudget(budget1);

        budgetComponent2.setCategory("purchases");
        budgetComponent2.setCurrency(10000.00);
        budgetComponent2.setCurrencyFormat("Indian Rupee");
        budgetComponent2.setFrequency("weekly");
        budgetComponent2.setCreatedAt("2020-03-11");
        budgetComponent2.setCreatedBy("bruce");
        budgetComponent2.setModifiedAt("2020-05-16");
        budgetComponent2.setModifiedBy("bruce");
        budgetComponent2.setDeleted(false);
        budgetComponent2.setActive(false);

        Budget budget2 = new Budget();
        budget2.setStartDate("2020-03-01");
        budget2.setEndDate("2020-11-01");
        budget2.setFrequency("monthly");
        budget2.setCreatedAt("13:45");
        budget2.setCreatedBy("bruce");
        budget2.setModifiedAt("15:45");
        budget2.setModifiedBy("bruce");
        budget2.setDeleted(false);
        budget2.setActive(false);

        budgetComponent2.setBudget(budget2);
    }

    @Test
    public void testFindAll(){
        List<BudgetComponent> budgetComponentsList = new ArrayList<>();

        budgetComponentsList.add(budgetComponent1);
        budgetComponentsList.add(budgetComponent2);

        //Mocking
        Mockito.when(budgetComponentRepository.findAll()).thenReturn(budgetComponentsList);

        //Test
        List<BudgetComponentDTO> totalBudgetComponents = budgetComponentServiceImpl.findAll();
        assertThat(2).isEqualTo(totalBudgetComponents.size());
    }

    @Test
    public void testFindById(){
        Mockito.when(budgetComponentRepository.findById(budgetComponent1.getId())).
                thenReturn(Optional.of(budgetComponent1));
        BudgetComponentDTO budgetComponentDTO = budgetComponentServiceImpl.findById(budgetComponent1.getId());
        assertThat(budgetComponent1).isEqualToComparingFieldByField(budgetComponentMapper.convertToEntity(budgetComponentDTO));
    }

    @Test
    public void testSave(){
        Mockito.when(budgetComponentRepository.save(budgetComponent1)).thenReturn(budgetComponent1);
        assertThat(budgetComponentMapper.convertToDTO(budgetComponent1)).isEqualToComparingFieldByField(budgetComponentServiceImpl.save(budgetComponentMapper.convertToDTO(budgetComponent1)));
    }

    @Test
    public void testDeleteById(){
        budgetComponentServiceImpl.deleteById(budgetComponent1.getId());
        Mockito.verify(budgetComponentRepository,Mockito.times(1))
                .deleteById(budgetComponent1.getId());
    }
}














